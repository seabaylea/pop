...
public class ServerMealResource {
    private let adapter: ServerMealAdapter
    private let path = "/api/ServerMeals"
    private let pathWithID = "/api/ServerMeals/:id"

    init(factory: AdapterFactory) throws {
        adapter = try factory.getServerMealAdapter()
    }
    ...
    private func handleCreate(request: RouterRequest, response: RouterResponse, next: @escaping () -> Void) {
        ...
        do {
            let model = try ServerMeal(json: json)
